...
export default function ClaimPage() {
    // const params = useParams(); // Removed in favor of hash
    ...
    useEffect(() => {
        // Parse hash fragment for secret
        // Check if we are in browser environment
        if (typeof window !== 'undefined') {
            const hash = window.location.hash;
            if (hash && hash.length > 1) {
                const secret = hash.slice(1); // Remove the '#'
                setNoteString(secret);
            }
        }
    }, []);
    ...
    useEffect(() => {
        if (noteString) {
            try {
                // Ensure noteString is properly URL decoded before base64 decoding
                const decodedString = decodeURIComponent(noteString);
                const secretKey = Buffer.from(decodedString, 'base64');
